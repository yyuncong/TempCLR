...
class AlignLocalTask(Task):
    ...
    def __call__(self, model, sample):
        ...
        if model.training and len(action_sequence) > 1 and len(video_seq) > 1:
            alignment_distances = self.query_to_logits(pooled_text, video_seq, action_sequence, non_empty_targets)
    ...
    def query_to_logits(self, query, keys, action_sequence, video_indices):
        ...
        idx_f = np.linspace(0, keys.shape[-2] - 1, num=min(keys.shape[-2], action_sequence.shape[0]*video_sampling_multiplier))
        ...
        video_indices = torch.cumsum(torch.cat([torch.tensor([0]).cuda(), counts], dim=0), dim=0)
        ...
        while len(negative_keys) < total_sample_size - 1:
            ...
            if not torch.equal(action_sequence, negative_action_sequence):
                negative_key = torch.index_select(origin_dists, 0, negative_action_sequence)

                if inner_shuffling:
                    origin_indices = [i for i in range(keys.shape[-2])]
                    inner_shuffled_indices = []
                    for i in range(len(video_indices) - 1):
                        step_indices = origin_indices[video_indices[i]: video_indices[i + 1]]
                        random.shuffle(step_indices)
                        inner_shuffled_indices.extend(step_indices)
                    negative_key = torch.index_select(negative_key, 1, torch.tensor(inner_shuffled_indices).cuda())
