...
class DistributedVectorPool(VectorPool):
    ...
    def load(self, local_rank):
        hidden_states = np.load(
            os.path.join(self.out_dir, "hidden_state" + str(local_rank) + ".npy")
        )

        with open(
            os.path.join(self.out_dir, "video_id" + str(local_rank) + ".pkl"), "rb"
        ) as fr:
    ...
    def save(self):
        ...
        assert len(hidden_states) == len(self.video_ids), "{}, {}".format(
            len(hidden_states), len(self.video_ids)
        )
        local_rank = (
        ...
        with open(
            os.path.join(self.out_dir, "video_id" + str(local_rank) + ".pkl"), "wb"
        ) as fw:
...
class TextClipVectorPool(VectorPool):
    ...
    def __call__(self, sample, **kwargs):
        ...
        if hasattr(self, "retriver"):
            ...
            self.retriver.add(
                sample["pooled_text"].cpu().numpy().astype("float32"), text_meta
            )
        else:
            raise NotImplementedError
...
class MMClipVectorPool(VectorPool):
    ...
    def __call__(self, sample, **kwargs):
        ...
        video_ids = [
            video_id for video_id in sample["video_id"] for _ in range(subsample_size)
        ]
