...
def OTAM_cum_dist(dists, lbda=0.1):
    ...
    # top row
    for m in range(1, dists.shape[3]):
        cum_dists[:,:,0,m] = dists[:,:,0,m] - lbda * torch.logsumexp(-cum_dists[:, :, 0, m - 1].unsqueeze(dim=-1) / lbda, dim=-1)

    # remaining rows
    for l in range(1, dists.shape[2]):
        # first non-zero column
        cum_dists[:, :, l, 1] = dists[:, :, l, 1] - lbda * torch.logsumexp(
            torch.cat([-cum_dists[:, :, l - 1, 0].unsqueeze(dim=-1) / lbda, -cum_dists[:, :, l - 1, 1].unsqueeze(dim=-1) / lbda, -cum_dists[:, :, l, 0].unsqueeze(dim=-1) / lbda], dim=-1),
            dim=-1
        )
        ...
        for m in range(2, dists.shape[3] - 1):
            cum_dists[:, :, l, m] = dists[:, :, l, m] - lbda * torch.logsumexp(
                torch.cat([-cum_dists[:, :, l - 1, m - 1].unsqueeze(dim=-1) / lbda, -cum_dists[:, :, l, m - 1].unsqueeze(dim=-1) / lbda], dim=-1),
                dim=-1
            )

        # last column
        cum_dists[:, :, l, -1] = dists[:, :, l, -1] - lbda * torch.logsumexp(
            torch.cat([-cum_dists[:, :, l - 1, -2].unsqueeze(dim=-1) / lbda, -cum_dists[:, :, l - 1, -1].unsqueeze(dim=-1) / lbda, -cum_dists[:, :, l, -2].unsqueeze(dim=-1) / lbda], dim=-1),
            dim=-1
        )
